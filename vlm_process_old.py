...
# ================= 关键修复：加载 .env 文件 =================
# 获取当前脚本所在目录 (即 /home/xingyu/projects/VLM_Grasp_Interactive)
...
def generate_robot_actions(user_command, image_input=None):
    ...
    if not API_KEY:
        raise ValueError("❌ 未找到环境变量 QWEN_API_KEY，请在终端 export 或使用 .env 文件")
    client = OpenAI(api_key=API_KEY, base_url="https://dashscope.aliyuncs.com/compatible-mode/v1")

    system_prompt = textwrap.dedent("""\
    你是一个精密机械臂视觉控制系统。你的任务是分析图像，根据用户指令找到目标物体，并返回坐标。

    请在内部完成图像识别和指令匹配，但在输出时必须严格遵守以下规则：

    1. **绝对禁止**输出任何“分析阶段”、“指令解析”、“检测到的物体列表”等中间思考过程。
    2. **直接输出**给用户的自然语言回复，紧接着换行输出JSON数据。
    3. 你的输出内容将直接用于语音播报和机械臂控制，因此必须干净、简洁。

    【输出格式要求】
    第一行：自然语言响应（简短、俏皮、只确认被选中的目标，不要啰嗦）。
    第二行：标准 JSON 对象（不要使用 markdown 代码块，不要标明 json 标签，直接以 { 开头）。
    ...
    用户指令：抓取那个红色的苹果
    你的回答：
    收到！已经锁定那个红彤彤的苹果啦，准备抓取。
    {
    "name": "红色的苹果",
    "bbox": [484, 241, 540, 308]
    }

    请根据当前图像和指令："{user_instruction}"，生成回答。
    """)
...
def choose_model():
    ...
    model_weight = '/home/xingyu/projects/VLM_Grasp_Interactive/sam_b.pt'
    overrides = dict(
        task='segment',
        mode='predict',
        # imgsz=1024,
...
def segment_image(image_input, output_mask='mask1.png'):
    ...
    if bbox:
        try:
            ...
            # 【已删除】原有的 cv2.putText 文字绘制代码
            
            # 3. 保存图片
            vis_filename = "debug_detection.jpg"
            cv2.imwrite(vis_filename, vis_img)
            print(f"🖼️  [调试] 检测框已可视化保存为: {vis_filename}")
    ...
    if bbox:
        ...
        points_prompt = [[center_x, center_y]]
        labels_prompt = [1] 

        results = predictor(bboxes=[bbox], points=points_prompt, labels=labels_prompt)
